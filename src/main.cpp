        ...
        if (length && length > 2 && data[0] == '4' && data[1] == '2')
        {
            ...
            if (s != "")
            {
                ...
                if (event == "telemetry")
                {
                    ...
                    // Start at the last path end point if some points are left
                    if (prev_size > 0)
                    {
                        car_s = end_path_s;
                    }
                    ...
                    // Loop over each tracked car
                    for (int indexOfCar = 0; indexOfCar < sensor_fusion.size(); ++indexOfCar)
                    {
                        float d = sensor_fusion[indexOfCar][6];
                        double vx = sensor_fusion[indexOfCar][3];
                        double vy = sensor_fusion[indexOfCar][4];
                        double check_speed = sqrt(vx * vx + vy * vy);
                        double check_car_s = sensor_fusion[indexOfCar][5];
                        ...
                        if (left_lane >= 0 && left_available && d < (2 + 4 * left_lane + 2) &&
                            d > (2 + 4 * left_lane - 2))
                        {
                            if (((check_car_s > car_s) && ((check_car_s - car_s) < safe_distance)) ||
                        ...
                        if (right_lane <= 2 && right_available && d < (2 + 4 * right_lane + 2) &&
                            d > (2 + 4 * right_lane - 2))
                        {
                            if (((check_car_s > car_s) && ((check_car_s - car_s) < safe_distance)) ||
                                ((check_car_s < car_s) && ((car_s - check_car_s) < safe_distance)))
                    ...
                    // Transform the waypoints to local car coordinates
                    // The last waypoint will be at the coordinate systems origin
                    ...
                    for (int k = 0; k < 50 - previous_path_x.size(); ++k)
                    {
